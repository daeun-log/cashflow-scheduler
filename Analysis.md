...
- 실행 환경에 독립적인 로컬 프로그램으로 개발하며, Java의 Serialization 기술을 통해 복잡한 DB 없이 데이터 영속성을 보장한다
...
|:---|:---|
|Summary| 새로운 사용자가 시스템에 계정 파일과 고유 금융 데이터 파일을 생성하고, 접근 권한을 마련하는 기능|
|Scope|자금 흐름 스케줄러|
|Level|User Level|
|Author|Lee daeun|
...
|Success Post Condition|새로운 계정 정보를 포함한 사용자 파일 및 금융 데이터 파일이 로컬 디렉토리에 생성되고, 로그인 화면으로 전환됨|
|Failed Post Condition|계정 데이터 파일이 생성되지 않고 회원가입 화면에 머묾|

|MAIN SUCCESS SCENARIO||
|:---|:---|
|Step|Action|
|1|사용자가 회원가입 화면에서 사용자 정보를 입력하고 가입요청을 한다|
|2|사스탬은 압력된 ID를 기반으로 로컬 디렉토리에 해당 ID의 데이터 파일(예: `[ID]_data.dat`)이 이미 존재하는지 확인한다|
|3|모두 완료되면 입력된 계정 정보 및 초기 자금 흐름 리스트를 담은 고유 파일(`[ID]_data.dat`)을 동적으로 생성 및 저장하고, 회원가입 성공 메시지를 출력한다|
...
|GENERAL CHARACTERISTICS||
|:---|:---|
|Summary|기존 사용자가 로컬 디렉토리의 파일 인증을 통해 시스템에 접속하여 접근 권한 획득하는 기능|
|Scope|자금 흐름 스케줄러|
|Level|User Level|
|Author|Lee daeun|
|Last Updated|2026-05-07|
|Status|Analysis|
|Primary Actor|User|
|Preconditions|해당 사용자의 고유 데이터 파일(`[ID]_data.dat`)이 로컬 디렉토리에 존재해야 한다|
|Trigger|로그인 화면에서 ID와 PW를 입력하고, 로그인 버튼을 클릭할 때|
|Success Post Condition|로그인에 성공해서 해당 ID 파일의 금융 데이터를 시스템 메모리에 로드하고, 자신의 스케줄러 화면에 진입|
...
|2|시스템은 로컬 디렉토리에서 입력된 ID와 매핑되는 데이터 파일(`[ID]_data.dat`)을 탐색하여 파일 내 저장된 정보와 비교하여 일치 여부를 판단한다|
|3|정보가 일치하면 해당 파일의 데이터를 읽어와 활성화하고, 사용자의 스케줄러 화면을 띄운다|
...
|GENERAL CHARACTERISTICS||
|:---|:---|
|Summary|사용자가 시스템 접속을 종료하기 전 메모리의 데이터를 최종 저장하고 접근 권한을 해제하는 기능|
|Scope|자금 흐름 스케줄러|
|Level|User Level|
|Author|Lee daeun|
|Last Updated|2026-05-07|
|Status|Analysis|
|Primary Actor|User|
|Preconditions|시스템에 정상적으로 로그인되어 데이터 파일이 로드된 상태여야 한다|
|Trigger|로그아웃 버튼을 클릭할 때|
|Success Post Condition|자금 흐름 데이터가 파일에 최종 직렬화되어 저장되고, 접속 상태가 해제되어 로그인 화면으로 돌아감|
|Failed Post Condition|로그아웃 및 파일 최종 저장에 실패함|

|MAIN SUCCESS SCENARIO||
|:---|:---|
|Step|Action|
|1|사용자가 로그아웃을 요청한다|
|2|시스템은 메모리 내의 자금 흐름 변경 사항을 해당 사용자의 파일(`[ID]_data.dat`)에 직렬화하여 덮어쓰고, 사용자의 겁근권한을 회수한다|
...
#### 2.2.5.1. Use Case #15 : Check Overdraft Alert
...
#### 2.2.5.2. Use Case #16 : Calculate Depletion Data
...
#### 2.2.6.1. Use Case #17 : Save Data

|GENERAL CHARACTERISTICS||
|:---|:---|
|Summary|입력 및 변경된 모든 금융 이벤트 데이터를 자바 직렬화를 통해 즉시 로컬 파일에 영구 저장하며, 저장 실패 시 시스템 종료 단계에서 재시도를 유도하는 안전망 기능|
|Scope|자금 흐름 스케줄러|
...
|Status|Analysis|
|Primary Actor|User|
|Preconditions|시스템에 신규 등록, 수정, 삭제 등 데이터의 변동(트랜잭션)이 발생하거나 종료 이벤트가 감지되어야 한다|
|Trigger|데이터 변동(추가/수정/삭제) 발생 시, 수동 저장 버튼 클릭 시, 또는 시스템 종료(X) 버튼 클릭 시|
|Success Post Condition|메모리 상의 전체 데이터 리스트가 로컬 파일에 완벽히 동기화되어 저장되며, 안전하게 프로그램이 종료됨|
|Failed Post Condition|저장 실패 상태가 유지되며, 종료 프로세스가 중단됨|

|MAIN SUCCESS SCENARIO||
|:---|:---|
|Step|Action|
|1|사용자가 데이터를 추가/수정/삭제하거나 명시적 저장 버튼을 클릭한다|
|2|시스템은 백그라운드에서 데이터를 파일에 직렬화하여 덮어쓰고, '데이터 변경 감지 플래그(isDirty)'를 `false`로 유지한다|
|3|사용자가 시스템 종료를 요청하면, 플래그가 `false`이므로 즉시 안전하게 종료한다|

|EXTENSION SCENARIO||
|:---|:---|
|Step|Branching Action|
|1|데이터 변동 후 파일 저장 과정에서 디스크 오류/권한 부족 등으로 저장에 실패한 경우<br>1a.1 시스템은 데이터 변경 감지 플래그(isDirty)를 `true`로 설정하고 "자동 저장에 실패했습니다" 알림을 띄운다|
|2|사용자가 시스템 종료(X) 버튼을 클릭한다<br>2a. 데이터 변경 감지 플래그(isDirty)가 `true` (저장되지 않은 메모리 데이터 존재)인 경우<br>2a.1 시스템은 종료를 중단하고, "저장되지 않은 데이터가 있습니다. 다시 저장을 시도하고 종료하시겠습니까?" 경고 팝업을 출력한다<br>2a.2 사용자가 '예(Retry Save)'를 선택하면 저장을 재시도한 후 성공 시 종료한다<br>2a.3 사용자가 '아니오(Discard)'를 선택하면 데이터 유실을 감수하고 그대로 종료한다<br>2a.4 사용자가 '취소(Cancel)'를 선택하면 종료를 취소하고 메인 화면으로 복귀한다|

|RELATED INFORMATION||
|:---|:---|
|Performance|<=5|
|Frequency|데이터 변동 및 시스템 종료 시|
|Concurrency||
|Due Date||
|Etc||
...
### 3.8. Timeline
유저가 등록한 내역들을 날짜순으로 정렬하여 전체적인 자금 흐름과 누적 예상 잔액을 표 형태로 조회하는 기능이 담긴 클래스이다. 데이터 연산 중 잔액이 0원 미만으로 떨어지는 시점(자금 고갈 예정일)을 백그라운드에서 스캔하고 도출하는 분석 로직을 함께 포함한다.
ex) 날짜순 정렬, 누적 예상 잔액 연산, 마이너스 잔액 발생 시점 탐지 등

### 3.9. Alert
타임라인 연산 결과(Timeline 클래스의 연산)를 바탕으로 잔액 부족 위험이나 고갈 예정일이 감지될 경우, 사용자에게 시각적 알림을 제공하는 기능이 담긴 클래스이다.
ex) 표 내 마이너스 잔고 행 붉은색 강조, 위험 날짜 경고 출력 등

### 3.10. File Management
자바의 객체 직렬화를 활용하여 시스템에 입력 및 변경된 모든 금융 이벤트 컬렉션 데이터를 해당 사용자의 로컬 고유 파일(`[ID]_data.dat`)에 즉시 영구 저장하고, 프로그램 시작 시 완벽히 복구하는 기능이 담긴 클래스이다.
ex) 컬렉션 데이터 로컬 파일 저장, 프로그램 재시작 시 데이터 불러오기 등
...
| :--- | :--- |
| 타임라인 (Timeline) | 시스템에 등록된 수입 및 지출 내역들이 날짜순으로 정렬되어, 전체적인 자금 흐름과 일별 누적 예상 잔액을 직관적인 표(Table/List) 형태로 한눈에 파악할 수 있도록 제공되는 화면 |
| 단발성 지출 (Expense) | 정기적이지 않고 일회성으로 발생하는 소비 |
| 고정 지출 (Fixed Expense) | 구독료, 교통비, 통신비 등 매월 정기적인 주기로 발생하는 예상 지출 |
| 오버드래프트 (Overdraft) | 타임라인 연산 결과, 특정 시점의 예상 잔액이 0원 미만으로 떨어져 마이너스 잔고가 되는 상태 (화면상에 붉은색 등 시각적 경고로 표시됨) |
| 로컬 데이터 파일 (.dat) | 외부 서버나 데이터베이스 없이, 개인 PC의 하드디스크에 직접 생성되어 사용자의 금융 데이터를 독립적이고 안전하게 보관하는 파일 |
| 객체 직렬화 (Serialization) | 자바(Java) 프로그램 메모리에 있는 자금 흐름 리스트 데이터를 로컬 데이터 파일에 기록하고 읽어오기 위해 0과 1의 데이터 형태로 변환/복원하는 기술 |
