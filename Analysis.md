...
| :--- | :--- |
|타임라인(캘린더)|시스템에 등록된 수입 및 지출 내역들이 날짜순으로 정렬되어 전체적인 자금 흐름과 일별 잔액을 한눈에 파악할 수 있도록 제공되는 화면 
|단발성지출(Expense)|정기적이지 않고 일회성으로 발생하는 소비|
|고정 지출 (Fixed Expense)|구독료, 교통비, 통신비 등 매월 정기적인 주기로 발생하는 예상 지출|
|오버드래프트 (Overdraft)|시스템의 연산 결과, 특정 시점의 예상 잔액이 0원 미만으로 떨어져 마이너스 잔고가 되는 상태|
|시뮬레이션 (Simulation)|실제 데이터베이스를 수정하지 않고, 가상의 지출 이벤트를 임시로 입력해 보며 미래의 잔액 변화를 사전에 확인해 보는 테스트 기능|
